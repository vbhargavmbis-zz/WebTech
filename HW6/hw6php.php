...
<meta charset="UTF-8">
 ...
 //  echo $_POST["query"];
   ...
   else
   {
    ...
    if($_POST["type"]=="artists")
	{
     $url = "http://www.allmusic.com/search/artists";
	 
	 
     $html = file_get_contents("http://www.allmusic.com/search/".$_POST["type"]."/".$query); 
  
     if(preg_match("/<table.*?>.*?<\/[\s]*table>/s", $html, $table_html))
	 {
	         echo "<div align=\"center\"><h1>Search Result</h1>";
	 echo "<b>\"".$_POST["query"]."\" of type \"".$_POST["type"]."\"</b><br><br>";
 ...
 //   preg_match("/<div class=\"image\">(.*?)<\/div>/s", $matches[1][0], $div_matches);
   ...
   echo "</table>";
	 ...
	 {
	   echo "<div align=\"center\"><h1>Search Result</h1>";
	   echo "<b>\"".$_POST["query"]."\" of type \"".$_POST["type"]."\"<br>";
	   echo "<h2>No Discography found!</h2></div>";
	 }
	}
	//END FOR ARTISTS
   else if($_POST["type"]=="albums")// FOR ALBUMS!!!!!!!!!!!!!!!	
	{
 ...
 //   preg_match("/<div class=\"image\">(.*?)<\/div>/s", $matches[1][0], $div_matches);
  ...
  else if($_POST["type"]=="songs")// FOR SONGS 	SONGS	SONGS	SONGS	SONGS	!!!!!!!!!!!!!!!	
	{
